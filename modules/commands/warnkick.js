  ...
  */
  ...
  if(args[0] == "view") {
  	if(!args[1]) {
  		...
  		if(!mywarn) return api.sendMessage('⚡️Bạn chưa bị cảnh cáo lần nào', threadID, messageID);
  		var num = 1;
  		for(let reasonwarn of mywarn) {
  			msg += `Lần ${num}: reasonwarn\n`;
  		}
  		api.sendMessage(`⚡️Bạn đã bị cảnh cáo ${mywarn.length} lần:\n\n${msg}`, threadID, messageID);
  	}
  		...
  		allwarn == "" ? api.sendMessage("⚡️Nhóm bạn chưa có ai bị cảnh cáo", threadID, messageID) : api.sendMessage("⚡️Danh sách những thành viên đã bị cảnh cáo:\n\n"+allwarn, threadID, messageID);
  	}
  }
  	...
  	var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('⚡️Chỉ qtv nhóm mới có thể sử dụng lệnh unban!', threadID, messageID);
