...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		...
		return api.sendMessage({body: ` 📝 Số tin nhắn:  ${Date.now() - timeStart} `, attachment: fs.createReadStream(pathRankCard, {'highWaterMark': 128 * 1024}) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
	}
	if (mention.length == 1) {
