...
module.exports.getInfo = async (uid, Currencies) => {
 ...
 return { level, expCurrent, expNextLevel, EXP: point };
}
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
 ...
 if (args.length == 0) {
  ...
  return api.sendMessage({body: `📝 Số tin nhắn: ${point.EXP} ✅.` ,attachment: fs.createReadStream(pathRankCard, {'highWaterMark': 128 * 1024}) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
 }
 if (mention.length == 1) {
  ...
  return api.sendMessage({body: `📝${point.EXP} tin nhắn✅` ,attachment: fs.createReadStream(pathRankCard) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
 }
 if (mention.length > 1) {
