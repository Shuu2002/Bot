module.exports.config = {
	...
	hasPermssion: 0,
	credits: "Vinhbeat",
	description: "Kêu bot hát với từ khóa bạn muốn",
	commandCategory: "Vinhbeat",
	usages: "[Nhập nội dung cần voice]",
	cooldowns: 10,
	dependencies: {
		"ytdl-core": "",
		"simple-youtube-api": "",
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	api.sendMessage(`Đang xử lý voice này\nXin Vui lòng Đợi vài giây !`, event.threadID, (err, info) =>
	...
	try {
		...
		api.sendMessage("Xin lỗi, Tôi không thể xử lý yêu cầu của bạn!", event.threadID, event.messageID);
	}
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('Phải nhập tên bài hát bạn muốn', event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.m4a`).size > 26214400) return api.sendMessage('File quá nặng', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
		...
		try {
			...
			if (error.statusCode == "404") return api.sendMessage("Xin lỗi, Tôi không tìm thấy bài nhạc của bạn", event.threadID, event.messageID);
		...
		try {
			...
			return api.sendMessage(`🎼 Có ${link.length} kết quả trùng với từ khoá tìm kiếm của bạn: \n${msg}\nHãy trả lời tin nhắn này với số bài hát của bạn\nThời Gian Bài Hát Tối Đa Là 5 phút!`, event.threadID,(error, info) => global.client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link }), event.messageID);
		}
		catch (error) {
			api.sendMessage("Error " + error.message, event.threadID, event.messageID);
		}
